...
class DealerEditorApp:
    """
    Main application class for the Dealer Editor.
    Handles UI creation, data management, and file operations for dealer data
    stored in JSON format, adhering to a specific schema and preserving
    other top-level JSON keys.
    """
    ...
    def _create_main_layout(self):
        """Creates the main paned window layout for dealer list and details."""
    ...
    def deals_from_text(self, text_widget):
        lines = self.list_from_text(text_widget); parsed_deals = []
    ...
    def new_file(self, event=None):
        if messagebox.askyesno("Confirm New File", "Discard all current data and start a new file?"):
            self.data = {"dealers": []}
            self.other_top_level_keys = {} 
            self.current_dealer_index = -1
            self.file_path = None
        ...
        try:
            ...
            if dealers_list is not None and isinstance(dealers_list, list):
                ...
                if not self.file_path and not self.data["dealers"]: 
                    ...
                    if messagebox.askyesno("Load Single Dealer", f"'{os.path.basename(path)}' appears to be a single dealer object. Merge it into the current data? (Other top-level keys from current session will be kept)"):
                        self.data['dealers'].append(loaded_full_structure) 
            else:
                messagebox.showerror("Load Error", "Invalid JSON content. Expected a 'dealers' list or a single dealer object with a 'name' field.")
                return
        ...
        for filepath in files_to_combine:
            try:
                ...
                if isinstance(file_content.get("dealers"), list): 
                    ...
                    errors_loading_files.append(f"No 'dealers' list or dealer 'name': {os.path.basename(filepath)}")
                    continue
    ...
    def merge_dealers(self):
        if not self.data.get("dealers") and not self.other_top_level_keys: 
             if not messagebox.askyesno("Merge Warning", "No data is currently loaded in the editor. Would you like to 'Open' the selected file instead of merging?"):
                 return
             else: 
                 self.load_json() 
                 return
    ...
    def _save_dealer_data(self, index, show_success=True):
         try:
            ...
            dialogue_data = {}
            for field, widget in self.dialogue_text_widgets.items():
                dialogue_data[field] = self.list_from_text(widget)
    ...
    # --- Drug Section Management (Schema Aligned) ---
    ...
    # --- Quality Section Management (Schema Aligned) ---
    ...
    # --- Effect Section Management (Schema Aligned) ---
    ...
    # --- Shipping Section Management (Schema Aligned) ---
