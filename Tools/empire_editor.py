...
class DealerEditorApp:
    ...
    def create_dealer_details_widgets(self, parent_frame):
        ...
        # --- Debt Frame ---
        self.debt_frame = ttk.LabelFrame(parent_frame, text="Debt", padding=(10, 5))
        self.debt_frame.grid(row=current_row, column=0, padx=5, pady=5, sticky="ew"); current_row += 1
        self.debt_frame.columnconfigure(1, weight=1)
        self.debt_enabled_var = tk.BooleanVar()
        self.debt_enabled_check = ttk.Checkbutton(self.debt_frame, text="Enable Debt", variable=self.debt_enabled_var, command=self._toggle_debt_widgets_state)
        self.debt_enabled_check.grid(row=0, column=0, columnspan=2, padx=5, pady=(0, 5), sticky="w")
        self.debt_total_entry = self._create_label_entry_pair(self.debt_frame, "Total Debt (int):", 1)
        self.debt_interest_rate_entry = self._create_label_entry_pair(self.debt_frame, "Interest Rate (int/float):", 2)
        self.debt_day_multiple_entry = self._create_label_entry_pair(self.debt_frame, "Day Multiple (int):", 3)
        self.debt_day_exponent_entry = self._create_label_entry_pair(self.debt_frame, "Day Exponent (int/float):", 4)
        self.debt_product_bonus_entry = self._create_label_entry_pair(self.debt_frame, "Product Bonus (int/float):", 5)
        self.debt_widgets = [self.debt_total_entry, self.debt_interest_rate_entry, self.debt_day_multiple_entry, self.debt_day_exponent_entry, self.debt_product_bonus_entry]
    ...
    def _toggle_debt_widgets_state(self):
        """Enables or disables the debt entry fields based on the checkbox."""
        state = 'normal' if self.debt_enabled_var.get() else 'disabled'
        for widget in self.debt_widgets:
    ...
    def load_selected_dealer(self, event):
        ...
        try:
            ...
            # Load Debt Data
            debt_data = dealer.get("debt")
            if debt_data:
                self.debt_enabled_var.set(True)
                self.clear_entry(self.debt_total_entry); self.debt_total_entry.insert(0, str(debt_data.get("total_debt", 0)))
                self.clear_entry(self.debt_interest_rate_entry); self.debt_interest_rate_entry.insert(0, str(debt_data.get("interest_rate", 0)))
                self.clear_entry(self.debt_day_multiple_entry); self.debt_day_multiple_entry.insert(0, str(debt_data.get("day_multiple", 0)))
                self.clear_entry(self.debt_day_exponent_entry); self.debt_day_exponent_entry.insert(0, str(debt_data.get("day_exponent", 0)))
                self.clear_entry(self.debt_product_bonus_entry); self.debt_product_bonus_entry.insert(0, str(debt_data.get("product_bonus", 0)))
            else:
                self.debt_enabled_var.set(False)
                for widget in self.debt_widgets: self.clear_entry(widget)
            self._toggle_debt_widgets_state()
    ...
    def _save_dealer_data(self, index, show_success=True):
         try:
            ...
            # Save Debt Data
            if self.debt_enabled_var.get():
                dealer["debt"] = {
                    "total_debt": self.safe_int(self.debt_total_entry.get()),
                    "interest_rate": self.safe_float(self.debt_interest_rate_entry.get()),
                    "day_multiple": self.safe_int(self.debt_day_multiple_entry.get()),
                    "day_exponent": self.safe_float(self.debt_day_exponent_entry.get()),
                    "product_bonus": self.safe_float(self.debt_product_bonus_entry.get())
                }
            else:
                dealer.pop("debt", None) # Remove debt key if it exists
